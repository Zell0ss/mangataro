#!/usr/bin/env python3
"""
Test script for AsuraScans scanlator plugin.

This script tests all three main methods of the AsuraScans plugin:
...
Usage:
    ...
    python scripts/test_asura_scans.py --headless  # Run browser in headless mode
    python scripts/test_asura_scans.py --search "solo leveling"  # Custom search term
"""
...
async def test_buscar_manga(scanlator: AsuraScans, search_term: str = "solo leveling"):
    """
    Test the buscar_manga method.

    Args:
        scanlator: AsuraScans instance
...
async def test_obtener_capitulos(scanlator: AsuraScans, manga_url: str):
    """
...
def test_parsear_numero_capitulo(scanlator: AsuraScans):
    """
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Test AsuraScans scanlator plugin')
    parser.add_argument(
        '--headless',
        action='store_true',
    ...
    logger.info("="*70)
    logger.info("AsuraScans Plugin Test Suite")
    logger.info("="*70)
